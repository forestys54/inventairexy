...
#'  only for Linux), this function can be used only from
...
create_indices_db <- function(xslt_path = NA,
  ...
  # we assume that a new version of indices.json is created at every new package update
  if (is.na(json_path)) {
    json_path <- file.path(system.file("data-raw/", package="inventairexy"), "indices.json")
  }
  if (system.file("data-raw/indices.json", package="inventairexy") == json_path) {
    if (force == FALSE) {
  ...
  parent_regex <- "\\{((?>[^{}]+)|(?R))*\\}"
  max_iter = 7 # maximum number of iterations for nested fractions
  ...
  for (sel_row in seq_len(nrow(s2_table))) {
    ...
    tmp_latex <- gsub("\u00B7", "*", tmp_latex) # replace middle point
  ...
  # # TODO2 check all the automatic parsings (maybe expression do not provide errors but they are different from originals)
  ...
  s2_table[grepl("BLUE",s2_formula_mathml_general),
           s2_formula := gsub("band_1","band_2",s2_formula)] # B1 to B2 (Blue)
